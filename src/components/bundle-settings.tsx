...
export const BundleSettings: React.FC<IProps> = ({ settings, setSettings, setActiveTierId}) => {
  const { bundleTitle, colorThemeSettings, tiers, shouldShowSavingsBadge, shouldShowGuarantee, shouldShowOriginalPrice, shouldShowShareButton } = settings
  ...
  const handleBundlePreviewModeChange = (id: string) => {
    setSettings({ ...settings, layout: id as 'default' | 'compact' })
    setActiveTierId(1)
  }
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col gap-1.5'>
        <span className='text-sm font-medium text-secondary/60'>Bundle Layout</span>

        <div className='flex gap-2'>
          {[
            { id: 'default', label: 'List', icon: <Icons.layoutList /> },
            { id: 'compact', label: 'Compact', icon: <Icons.layoutGrid /> },
          ].map(({ id, label, icon }) => (
            <button
              key={id}
              onClick={() => handleBundlePreviewModeChange(id)}
              className={cn(
                'flex-1 flex items-center justify-center gap-2 py-1.5 rounded-lg border-2 text-sm font-medium transition-colors',
                settings.layout === id
                  ? 'border-primary font-semibold bg-primary/10 text-primary'
                  : 'border-black/10 text-secondary/50 hover:text-primary'
              )}
            >
              {icon} {label}
            </button>
      ...
      <div className='flex flex-col gap-4'>
        ...
        {[
          ...
          { id: 'shouldShowShareButton', label: 'Show share button', value: shouldShowShareButton },
        ].map(({ id, label, value }) => (
